...
public class PackDeliveryManager {
    ...
    /** The pack id this player currently has selected, or NexusResourcePacksConfig.NONE_ID. Falls back to config defaults if nothing's been recorded yet, or if their old choice no longer exists in config.yml -- and falls all the way back to NONE_ID if even the configured default doesn't correspond to a real, currently-hosted pack, so a pack with no actual file behind it (not configured at all, or configured but missing its zip) can never repeatedly try and fail to send a phantom pack. */
    public String currentSelection(Player player) {
        String stored = preferences.get(player.getUniqueId());
        String candidate = (stored != null) ? stored : config.defaultPackId();

        if (NexusResourcePacksConfig.NONE_ID.equalsIgnoreCase(candidate)) return NexusResourcePacksConfig.NONE_ID;
        if (config.findPack(candidate) == null || !host.isReady(candidate)) return NexusResourcePacksConfig.NONE_ID;
        return candidate;
    }
