...
public class NexusResourcePacksPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        getServer().getPluginManager().registerEvents(new PackJoinListener(this, delivery), this);
        ...
        getLogger().info("NexusResourcePacks enabled with " + config.packs().size()
                + " pack(s) configured. Reminder: Resource Pack Manager's own autoHost should be set to "
                + "false for any pack it merges, so it isn't also trying to deliver on top of this plugin.");
    }
